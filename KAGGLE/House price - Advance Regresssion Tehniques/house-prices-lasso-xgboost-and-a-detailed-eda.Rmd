...
```{r}
train <- read.csv("~/Github/R_PRACTICE/KAGGLE/House price - Advance Regresssion Tehniques/train.csv", stringsAsFactors = F)
test <- read.csv("~/Github/R_PRACTICE/KAGGLE/House price - Advance Regresssion Tehniques/test.csv", stringsAsFactors = F)
```
...
a1 <- ggplot(all[!is.na(all$SalePrice)==T,], aes(x=Neighborhood, y=SalePrice)) +
  geom_histogram(stat = "summary", fun.y="median", fill="blue") +
  scale_y_continuous(breaks=seq(0,340000, by=50000), labels = comma) +
  theme(axis.text.x=element_text(angle=45, hjust=1)) +
  geom_hline(yintercept=median(all$SalePrice, na.rm = T), color="red", linetype = "dashed") +
  geom_label(stat="count", aes(label=..count.., y=..count..), size=3)

a2 <- ggplot(all, aes(x=Neighborhood)) +
  geom_bar(stat="count") +
...
ㅋㅋㅋ 아니에요 괜찮았어요! 
바쁜일은 다 끝났어요 ㅎㅎ 연구소 일 조금만 하면 돼요 
가은씨는 뭐하고 계셨나요?
...
피자 부럽읍니다
ㅋㅋㅋ 헐 우리 대학원 동지였군요
무슨 과에요? 저는 통계학과에요 ㅋㅋ 0350

오 통계학
학부때 심리
지금은 아주대 교육대학언에서 상담심리 전공!
아 뭔가너무 자기소개 느낌인뎈ㅋㅋㅋㅋㅋ
ㅋㅋㅋ일반 대학원 다니시는 거죠? 0356

ㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋ
제 차롄가요 ㅋㅋㅋㅋ
저는 학부랑 대학원 둘다 대구쪽에서 통계학 전공했어요 ㅋㅋ 일반대학원 맞아요 ㅋㅋ
심리학 오 뭔가 멋져요 
상담 심리면 어떤걸 하는 거에요?0405

상담교사 하고싶어서 전공
wee 클래서
여기 아니면 교육청 wee센터에서 일해요 ㅎㅎㅎ
저도 통계학은 잘 몰라서 ㅠㅠ 통계학은 어떤걸 하는 거에요??0425

아 교육 쪽 진로시구나~ 저도 가르치는거 좋아해서 선생님 하고 싶었었어요 ㅋㅋ
결정하기 쉬운 직업은 아니었을 텐데, 대단하네요!
음... 통계도 여러 분야가 있긴 한데, 기본적으로 데이터 형태나 그 데이터 분야에 맞게 분석하는 거에요.
내년부터 머신러닝이라는 분야쪽으로 취업 하려고 준비 중입니다 ㅋㅋㅋ 
지금 졸업까지는 많이 남으셨어요?? 0447

아 그렇구낭 ㅎㅎㅎ
저는 이제 2학기에요 ㅠㅠ
5학기 졸업이라서 아직 3학기 남았어요 ㅠㅠ,, 0452
언제 졸업하세요? ㅎㅎㅎ
아 그러고 보니까 제가 나이도 모르네요 ㅠㅠ
저는 24살이에요 ㅋㅋ 나이가 어떻게 되세요? 0453

아! 그렇네요 저는 27살 이에요 ㅋㅋ 졸업은 이번학기 막학기에요!
24살이시면 되게 빨리 들어가셨네요 0501


우아 부러워요 ㅠㅠ 0526
저는 휴학 안하고 졸업하면서 바로 대학원 들어가서 그런거 같아요 ㅋㅋㅋ





혹시 시간 되면 우리 한번 볼래요? 언제 괜찮으세요?
...
```{r}
#exercise
a1 <- ggplot(all[!is.na(all$SalePrice)==T,], aes(x=MSSubClass, y=SalePrice)) +
  geom_bar(stat="summary", fun.y="median", fill="blue") +
  geom_label(stat="count", aes(label = ..count.., y=..count..), size=3) +
  theme(axis.text.x=element_text(angle = 45, hjust=1)) +
  geom_hline(yintercept=median(all$SalePrice, na.rm=T), linetype="dashed", color="red") +
...
a2 <- ggplot(all, aes(x=MSSubClass)) +
  geom_bar(stat="count") +
  geom_label(stat="count", aes(label=..count..), size=3) +
  theme(axis.text.x = element_text(angle = 45, hjust=1))
...
b7 <- ggplot(data=all, aes(x=as.factor(BsmtCond))) +
        geom_histogram(stat='count')+ labs(xd='Rating of general condition')
...
```{r}
#exercise
a1 <- ggplot(all[!is.na(all$SalePrice)==T,], aes(x=as.factor(TotBathrooms), y=SalePrice)) +
  geom_point(stat = "identity", color = "blue") +
  scale_y_continuous(breaks = seq(0, 800000, by = 100000), label=comma) +
  geom_smooth(method = "lm", aes(group=1), se=F, color="black")

a2 <- ggplot(all, aes(x=as.factor(TotBathrooms))) + 
  geom_bar(stat="count")
...
```{r}
#exercise
ggplot(all[!is.na(all$SalePrice)==T,], aes(x=as.factor(IsNew),y=SalePrice)) +
  geom_bar(stat = "summary", fun.y="median", fill="violet") +
  geom_label(stat="count", aes(label=..count.., y=..count..)) +
  geom_hline(yintercept = median(all$SalePrice, na.rm=T), linetype="dashed", color="black") +
  scale_y_continuous(breaks=seq(0,250000, by=50000), label=comma)
...
```{r}
aa <- DFnumeric$LotFrontage
bb <- (aa-mean(DFnumeric$LotFrontage))/sd(DFnumeric$LotFrontage) # same to DFnorm$LotFrontage
# mean = "center", sd = "scale"
```
